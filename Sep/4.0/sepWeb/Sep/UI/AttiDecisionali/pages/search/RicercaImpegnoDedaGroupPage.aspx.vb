...
Partial Class RicercaImpegnoDedaGroupPage
    Inherits System.Web.UI.Page


#Region "PROPRIETA'"

    Private Property Impegni() As List(Of ParsecWebServices.ImpegnoModel)
        Get
            Return CType(Session("RicercaImpegnoDedaGroupPage_Impegni"), List(Of ParsecWebServices.ImpegnoModel))
        End Get
        Set(ByVal value As List(Of ParsecWebServices.ImpegnoModel))
            Session("RicercaImpegnoDedaGroupPage_Impegni") = value
        End Set
    End Property
    ...
    Protected Sub ImpegniGridView_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles ImpegniGridView.NeedDataSource
        If Me.Impegni Is Nothing Then
            ...
            Try
                ...
                If Not Request.QueryString("AnnoEsercizio") Is Nothing Then
                    postParameters.Add("AnnoImpegno", Request.QueryString("AnnoEsercizio"))
                    postParameters.Add("Esercizio", CInt(Me.AnniComboBox.SelectedValue))
                End If
                ...
                Dim service As New ParsecWebServices.DedaGroupService(String.Format(endPoint, "CercaImpegno"), parameter)

                Me.Impegni = service.QueryImpegni(postParameters)
            Catch ex As Exception
                ParsecUtility.Utility.MessageBox(ex.Message, False)
                Me.Impegni = New List(Of ParsecWebServices.ImpegnoModel)
            End Try
...
#Region "METODI PRIVATI"
    ...
    Private Sub SelezionaImpegno(ByVal item As Telerik.Web.UI.GridDataItem)
        Dim id As Integer = item.OwnerTableView.DataKeyValues(item.ItemIndex)("IdImpegno")
        Dim impegno As ParsecWebServices.ImpegnoModel = Me.Impegni.Where(Function(c) c.IdImpegno = id).FirstOrDefault
